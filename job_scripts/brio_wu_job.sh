...
ARGS=(
    ...
    -w 0.2
    --wextra 0.005,0.1
    -f x,y,z,rho,h,Bx,By,Bz,alpha_B,vx,vy,vz,u,p,divB
    -o "$OUT_DIR/dump.h5"
    -op "$OUT_DIR/profile"
    --resisitivity SLR
)
...
python "$REPO_ROOT/scripts/plot_shocktube.py" "$OUT_DIR/dump.h5" --all
